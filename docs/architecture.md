...
|---|---|
| `docendo.ingestion.scraper` | Discovers RBI PDFs and downloads them to `data/raw/`. |
| `docendo.ingestion.reader` | Extracts text via `pypdf`; falls back to MiniMax vision for scanned pages. |
| `docendo.retrieval.chunker` | Gigatoken wrapper. Chunks by token count (default 384, overlap 64). |
| `docendo.retrieval.embedder` | LiteLLM embedding client. Batched, retried, in-memory LRU cache. |
| `docendo.retrieval.store` | SQLite + FTS5 + sqlite-vector backend. Hybrid search via RRF. |
| `docendo.retrieval._internal` | Process-wide singleton retriever, cached on `(path, settings)`. |
| `docendo.retrieval.tools` | Four Pydantic AI tool functions: `search`, `fetch`, `recent`, `compare`. |
...
| `docendo.ui.app` | A/B chat demo. |
| `docendo.cli.checkup` | Local diagnostics. |

## Tool surface

| Tool | Inputs | Output | Backend calls |
|---|---|---|---|
| `search` | `query: str (1-512)`, `limit: int (1-20)` | `Results` (ranked hits) | FTS5 `bm25` + `vector_full_scan` + RRF fusion in Python |
| `fetch` | `id: str` (regex-validated) | `Document` (chunks in order) | `chunks` table filtered by `circular_id`, ordered by `chunk_index` |
| `recent` | `since: YYYY-MM-DD`, `limit: int (1-20)` | `list[Listing]` (first chunks) | Partial index `idx_chunks_first_recent` |
| `compare` | `id_a, id_b: str` | `PairResult` (two `Document`s) | Two `fetch` calls |
...
| `title` | TEXT | — |
| `text` | TEXT | chunk text (≤ `chunk_max_chars`) |
...
| `embedding` | BLOB | sqlite-vector `FLOAT32` of `vector_dims` |
...
- One `Store` per `(path, settings)` per process. Constructed via
  `functools.lru_cache` in `_internal.cached_store`.
...
- Reads use a per-thread connection so the WAL concurrent-readers
  benefit is actually realised. Writes still serialise through
  `_lock`.
- Embedding calls happen on the asyncio event loop (LiteLLM is
  async). SQLite writes are offloaded via `asyncio.to_thread` so they
...
- **Tokenizer fails to load**: `ConfigurationError` at the first call
  to `chunk`. Surface this early via `docendo checkup`.
